...
sys_socket_t WINS_OpenSocket (int port)
{
	...
	if (tcpipAvailable)
	{
		...
		Sys_Error ("Unable to bind to %s (%s)",
				WINS_AddrToString ((qsockaddr *) &address),
				socketerror(err));
...
============
*/
static int PartialIPAddress (const char *in, qsockaddr *hostaddr)
{
	char	buff[256];
	char	*b;
	...
	((sockaddr_in *)hostaddr)->sin_port = htons((unsigned short)port);
...
int WINS_Connect (sys_socket_t socketid, qsockaddr *addr)
{
	return 0;
}
...
int WINS_Read (sys_socket_t socketid, byte *buf, int len, qsockaddr *addr)
{
	socklen_t addrlen = sizeof(qsockaddr);
	int ret;

	ret = recvfrom (socketid, (char *)buf, len, 0, (sockaddr *)addr, &addrlen);
	if (ret == SOCKET_ERROR)
	{
		int err = SOCKETERRNO;
...
int WINS_Broadcast (sys_socket_t socketid, byte *buf, int len)
{
	...
	return WINS_Write (socketid, buf, len, (qsockaddr *)&broadcastaddr);
}

//=============================================================================

int WINS_Write (sys_socket_t socketid, byte *buf, int len, qsockaddr *addr)
{
	int	ret;
...
const char *WINS_AddrToString (qsockaddr *addr)
{
...
int WINS_StringToAddr (const char *string, qsockaddr *addr)
{
	...
	((sockaddr_in *)addr)->sin_port = htons((unsigned short)hp);
	return 0;
}

//=============================================================================

int WINS_GetSocketAddr (sys_socket_t socketid, qsockaddr *addr)
{
	socklen_t addrlen = sizeof(qsockaddr);
	in_addr_t a;

	memset(addr, 0, sizeof(qsockaddr));
	getsockname(socketid, (sockaddr *)addr, &addrlen);
...
int WINS_GetNameFromAddr (qsockaddr *addr, char *name)
{
	...
	hostentry = gethostbyaddr ((char *)&((sockaddr_in *)addr)->sin_addr,
...
int WINS_GetAddrFromName (const char *name, qsockaddr *addr)
{
	hostent *hostentry;
	...
	addr->qsa_family = AF_INET;
	((sockaddr_in *)addr)->sin_port = htons((unsigned short)net_hostport);
...
int WINS_AddrCompare (qsockaddr *addr1, qsockaddr *addr2)
{
	...
	if (((sockaddr_in *)addr1)->sin_addr.s_addr !=
	    ((sockaddr_in *)addr2)->sin_addr.s_addr)
		return -1;

	if (((sockaddr_in *)addr1)->sin_port !=
	    ((sockaddr_in *)addr2)->sin_port)
		return 1;
...
{
	((sockaddr_in *)addr)->sin_port = htons((unsigned short)port);
	return 0;
}
