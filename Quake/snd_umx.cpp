...
COMPILE_TIME_ASSERT(upkg_hdr, offsetof(upkg_hdr, gen) == UPKG_HDR_SIZE);
...
static int read_export (fshandle_t *f, const upkg_hdr *hdr,
...
static int read_typname(fshandle_t *f, const upkg_hdr *hdr,
			int idx, char *out)
{
	int i, s;
	long l;
...
static int probe_umx   (fshandle_t *f, const upkg_hdr *hdr,
			int32_t *ofs, int32_t *objsize)
{
	int i, idx, t;
	int32_t s, pos;
...
static int32_t probe_header (fshandle_t *f, upkg_hdr *hdr)
{
...
static qboolean S_UMX_CodecInitialize ()
{
	return true;
}
