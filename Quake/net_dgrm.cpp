...
static char *StrAddr (qsockaddr *addr)
{
	static char buf[34];
	byte *p = (byte *)addr;
	int n;
...
static in_addr	banAddr;
static in_addr	banMask;
...
static qsocket_t *_Datagram_CheckNewConnections ()
{
	...
	{
		in_addr_t	testAddr;
		testAddr = ((sockaddr_in *)&clientaddr)->sin_addr.s_addr;
...
static void _Datagram_SearchForHosts (qboolean xmit)
{
	...
	while ((ret = dfunc.Read (dfunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) > 0)
	{
		...
		Q_memcpy(&hostcache[n].addr, &readaddr, sizeof(qsockaddr));
...
static qsocket_t *_Datagram_Connect (const char *host)
{
	...
	if (ret == CCREP_ACCEPT)
	{
		Q_memcpy(&sock->addr, &sendaddr, sizeof(qsockaddr));
