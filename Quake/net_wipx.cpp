...
static sockaddr_ipx broadcastaddr;
...
int WIPX_Connect (sys_socket_t handle, qsockaddr *addr)
{
	return 0;
}
...
int WIPX_Read (sys_socket_t handle, byte *buf, int len, qsockaddr *addr)
{
	socklen_t addrlen = sizeof(qsockaddr);
	sys_socket_t socketid = ipxsocket[handle];
	int ret;

	ret = recvfrom (socketid, (char *)netpacketBuffer, len+4, 0, (sockaddr *)addr, &addrlen);
	if (ret == SOCKET_ERROR)
	{
		int err = SOCKETERRNO;
...
{
	return WIPX_Write (handle, buf, len, (qsockaddr *)&broadcastaddr);
}

//=============================================================================

int WIPX_Write (sys_socket_t handle, byte *buf, int len, qsockaddr *addr)
{
	sys_socket_t socketid = ipxsocket[handle];
	int ret;
	...
	ret = sendto (socketid, (char *)netpacketBuffer, len, 0, (sockaddr *)addr, sizeof(qsockaddr));
	if (ret == SOCKET_ERROR)
	{
		int err = SOCKETERRNO;
...
#undef DO
	...
	((sockaddr_ipx *)addr)->sa_socket = htons((unsigned short)val);
...
int WIPX_GetSocketAddr (sys_socket_t handle, qsockaddr *addr)
{
	sys_socket_t socketid = ipxsocket[handle];
	socklen_t addrlen = sizeof(qsockaddr);

	Q_memset(addr, 0, sizeof(qsockaddr));
	if (getsockname(socketid, (sockaddr *)addr, &addrlen) != 0)
	{
		int err = SOCKETERRNO;
